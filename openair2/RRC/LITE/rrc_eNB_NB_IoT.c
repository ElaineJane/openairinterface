...
//-----------------------------------------------------------------------------
{
  ...
  printf("[eNB] Generating rrcConnectionRelease-NB\n");
  ...
  MSC_LOG_TX_MESSAGE(
    ...
    printf("[eNB] RRC_MAC_DCCH_DATA_IND message(RRCConnectionRelease-NB) is sent to UE \n");
...
//Different from release: release cause and resumeid
void rrc_eNB_generate_RRCConnectionSuspend_NB_IoT(
  const protocol_ctxt_t* const ctxt_pP,
  rrc_eNB_ue_context_NB_IoT_t*          const ue_context_pP
)
//-----------------------------------------------------------------------------
{
  ...
  size = do_RRCConnectionSuspend_NB_IoT(ctxt_pP->module_id,
                                  buffer,
                    rrc_eNB_get_next_transaction_identifier_NB_IoT(ctxt_pP->module_id));
  // set release timer
  ue_context_pP->ue_context.ue_release_timer=1;
  ...
  LOG_I(RRC,
        PROTOCOL_RRC_CTXT_UE_FMT" Logical Channel DL-DCCH, Generate RRCConnectionSuspend-NB (bytes %d)\n",
        PROTOCOL_RRC_CTXT_UE_ARGS(ctxt_pP),
        size);

  LOG_D(RRC,
        PROTOCOL_RRC_CTXT_UE_FMT" --- PDCP_DATA_REQ/%d Bytes (rrcConnectionSuspend-NB MUI %d) --->[PDCP][RB %u]\n",
        PROTOCOL_RRC_CTXT_UE_ARGS(ctxt_pP),
        size,
        rrc_eNB_mui_NB_IoT,
        DCCH1_NB_IoT);//Through SRB1/or SRB1bis
  ...
  MSC_LOG_TX_MESSAGE(
    ...
    buffer,
    size,
    MSC_AS_TIME_FMT" rrcConnectionSuspend-NB UE %x MUI %d size %u",
    MSC_AS_TIME_ARGS(ctxt_pP),
    ue_context_pP->ue_context.rnti,
    rrc_eNB_mui_NB_IoT,
    size);
    ...
    itti_send_msg_to_task(TASK_RRC_UE, ctxt_pP->instance, message_p);
    printf("[eNB] RRC_MAC_DCCH_DATA_IND message(RRCConnectionSuspend-NB) is sent to UE \n");
...
//-----------------------------------------------------------------------------
{
 ...
 printf("[eNB] Generating RRCConnectionSetup_NB\n");
...
//  LogicalChannelConfig_NB_r13_t             *SRB1_logicalChannelConfig;
  ...
  //temperarily unused untill PHY build up
 // NB_IoT_DL_FRAME_PARMS *fp = get_NB_IoT_frame_parms(ctxt_pP->module_id,CC_id);
  ...
  for (cnt = 0; cnt < eNB_rrc_inst_NB_IoT[ctxt_pP->module_id].carrier[CC_id].Srb0.Tx_buffer.payload_size; cnt++) {
    LOG_F(RRC,"%02x ", ((uint8_t*)eNB_rrc_inst_NB_IoT[ctxt_pP->module_id].carrier[CC_id].Srb0.Tx_buffer.Payload)[cnt]);
  }
  ...
  //////////////////////////////////
    ...
    message_p = itti_alloc_new_message(TASK_RRC_ENB, RRC_MAC_CCCH_DATA_IND);
         RRC_MAC_CCCH_DATA_IND (message_p).frame     = get_NB_IoT_frame();
          RRC_MAC_CCCH_DATA_IND (message_p).sub_frame = get_NB_IoT_subframe();
          RRC_MAC_CCCH_DATA_IND (message_p).sdu_size  = eNB_rrc_inst_NB_IoT[ctxt_pP->module_id].carrier[CC_id].Srb0.Tx_buffer.payload_size;
          RRC_MAC_CCCH_DATA_IND (message_p).enb_index = 0;
          RRC_MAC_CCCH_DATA_IND (message_p).rnti      = get_NB_IoT_rnti();
          memset (RRC_MAC_CCCH_DATA_IND (message_p).sdu, 0, CCCH_SDU_SIZE);
          memcpy (RRC_MAC_CCCH_DATA_IND (message_p).sdu, eNB_rrc_inst_NB_IoT[ctxt_pP->module_id].carrier[CC_id].Srb0.Tx_buffer.Payload, eNB_rrc_inst_NB_IoT[ctxt_pP->module_id].carrier[CC_id].Srb0.Tx_buffer.payload_size);
          


    itti_send_msg_to_task(TASK_RRC_UE, ctxt_pP->instance, message_p);
    printf("[UE] RRC_MAC_CCCH_DATA_IND message(RRCConnectionSetup-NB) is sent to UE\n");
...
{
  //connection setup involve the establishment of SRB1 and SRB1bis (but srb1bis is established implicitly)
  //XXX: this message should go through SRB1 to see this--> uper_encode
  //XXX: they are assuming that 2 RLC-AM entities are used for SRB1 and SRB1bis

printf("[eNB] Generating RRCConnectionResume-NB!\n");
...
//  LogicalChannelConfig_NB_r13_t             *SRB1_logicalChannelConfig;
  ...
  int                     cnt;
// Attention: The following is copied from rrc_eNB_generate_RRCConnectionReconfiguration 
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
 ...
 // NB_IoT_DL_FRAME_PARMS *fp = get_NB_IoT_frame_parms(ctxt_pP->module_id,CC_id);
  ...
  size = do_RRCConnectionResume_NB_IoT(ctxt_pP,
                                                 buffer,
                                                 xid);

#ifdef RRC_MSG_PRINT
  LOG_F(RRC,"[MSG] RRC Connection Resume\n");

/*  for (cnt = 0; cnt < eNB_rrc_inst_NB_IoT[ctxt_pP->module_id].carrier[CC_id].Srb1.Srb_info.Tx_buffer.payload_size; cnt++) {
    LOG_F(RRC,"%02x ", ((uint8_t*)eNB_rrc_inst_NB_IoT[ctxt_pP->module_id].carrier[CC_id].Srb1.Srb_info.Tx_buffer.Payload)[cnt]);
  }*/
  ...
  //////////////////////////////////
    ...
    itti_send_msg_to_task(TASK_RRC_UE, ctxt_pP->instance, message_p);
    printf("[UE] RRC_MAC_CCCH_DATA_IND message(RRCConnectionResume-NB) is sent to UE\n");
...
/*  MSC_LOG_TX_MESSAGE(
    ...
    eNB_rrc_inst_NB_IoT[ctxt_pP->module_id].carrier[CC_id].Srb0.Tx_buffer.Header, // LG WARNING
    eNB_rrc_inst_NB_IoT[ctxt_pP->module_id].carrier[CC_id].Srb0.Tx_buffer.payload_size,
    MSC_AS_TIME_FMT" RRCConnectionResume-NB UE %x size %u",
    MSC_AS_TIME_ARGS(ctxt_pP),
    ue_context_pP->ue_context.rnti,
    eNB_rrc_inst_NB_IoT[ctxt_pP->module_id].carrier[CC_id].Srb0.Tx_buffer.payload_size);
  ...
  LOG_I(RRC,
        PROTOCOL_RRC_CTXT_UE_FMT" [RAPROC] Logical Channel DL-DCCH, Generating RRCConnectionResume-NB (bytes %d)\n",
        PROTOCOL_RRC_CTXT_UE_ARGS(ctxt_pP),
        eNB_rrc_inst_NB_IoT[ctxt_pP->module_id].carrier[CC_id].Srb0.Tx_buffer.payload_size);*/
...
//-----------------------------------------------------------------------------
{
...
#endif
  {
    ...
    //rrc_eNB_generate_RRCConnectionRelease_NB_IoT(ctxt_pP, ue_context_pP);
    //RRC Connection Suspend
    //rrc_eNB_generate_RRCConnectionSuspend_NB_IoT(ctxt_pP, ue_context_pP);
  }
}
...
//-----------------------------------------------------------------------------
{
	 ...
	 */
 printf("[eNB] Generating SecurityModeCommand-NB\n");
  uint8_t                             buffer[100];
  uint8_t                             size;
  ...
  MSC_LOG_TX_MESSAGE(
    ...
    itti_send_msg_to_task(TASK_RRC_UE, ctxt_pP->instance, message_p);
    printf("[UE] RRC_MAC_DCCH_DATA_IND message(SecurityModeCommand-NB) is sent to eNB\n");
...
/// was inside ITTI but I think it is needed
  ...
  MSC_LOG_TX_MESSAGE(
    ...
    message_p = itti_alloc_new_message(TASK_RRC_ENB, RRC_MAC_DCCH_DATA_IND);
         RRC_MAC_DCCH_DATA_IND (message_p).frame     = get_NB_IoT_frame();
          RRC_MAC_DCCH_DATA_IND (message_p).sub_frame = get_NB_IoT_subframe();
          RRC_MAC_DCCH_DATA_IND (message_p).sdu_size  = 13;  /////////////////////
          RRC_MAC_DCCH_DATA_IND (message_p).enb_index = 0;
          RRC_MAC_DCCH_DATA_IND (message_p).rnti      = get_NB_IoT_rnti();
          memset (RRC_MAC_DCCH_DATA_IND (message_p).sdu, 0, DCCH_SDU_SIZE);
          memcpy (RRC_MAC_DCCH_DATA_IND (message_p).sdu, buffer, 13);  //////////////////////////
          


    itti_send_msg_to_task(TASK_RRC_UE, ctxt_pP->instance, message_p);
    printf("[UE] RRC_MAC_DCCH_DATA_IND message(rrcConnectionReconfiguration_NB) has been sent to UE\n");
...
{ 
  ...
  if (eNB_rrc_inst_NB_IoT[ctxt_pP->module_id].carrier[CC_id].MIB_NB_IoT)
  {
    ...
    itti_send_msg_to_task(TASK_RRC_UE, ctxt_pP->instance, message_p);
    printf("[eNB] RRC_BCCH_BCH_DATA_IND message is sent to UE\n");
 ...
 //SIB1_NB_IoT
  ...
  if (eNB_rrc_inst_NB_IoT[ctxt_pP->module_id].carrier[CC_id].SIB1_NB_IoT){
    ...
    printf("[eNB] RRC_BCCH_DLSCH_DATA_IND (SIB 1) message is sent to UE\n");
  ...
  if (eNB_rrc_inst_NB_IoT[ctxt_pP->module_id].carrier[CC_id].SIB23_NB_IoT) {
    eNB_rrc_inst_NB_IoT[ctxt_pP->module_id].carrier[CC_id].sizeof_SIB23_NB_IoT = do_SIB23_NB_IoT(
          ...
          printf("[eNB] RRC_BCCH_DLSCH_DATA_IND (SIB 23) message is sent to UE\n");
         sleep(3); //wait for UE processing the SIB1 message
...
while ( eNB_rrc_inst_NB_IoT == NULL ) {
  ...
  //no CBA
  printf("[eNB] eNB processes the RRC_CONFIGURATION_REQ message and starts initializing system information!\n");
  //init SI
 //may be this should not be here
  ctxt.instance = 1;
  for (CC_id = 0; CC_id < MAX_NUM_CCs; CC_id++) {
...
//#   if defined(DISABLE_ITTI_XER_PRINT)...
  ...
  if (ul_ccch_msg_NB_IoT->message.present == UL_CCCH_MessageType_NB_PR_c1) {
    ...
    case UL_CCCH_MessageType_NB__c1_PR_rrcConnectionRequest_r13: //MSG3
      printf("[eNB] RRCConnectionRequest-NB Message received.\n");
...
//#else
  ...
  /*    LOG_I(RRC, PROTOCOL_RRC_CTXT_UE_FMT"CALLING RLC CONFIG SRB1bis and SRB1 (rbid %d, rbid %d)\n",
      ...
      MSC_LOG_TX_MESSAGE(
        ...
        MSC_AS_TIME_ARGS(ctxt_pP),
        ue_context_p->ue_context.rnti);
      //Following Interface with other layer temperarily removed!
...
//#endif //NO_RRM
    ...
    case UL_CCCH_MessageType_NB__c1_PR_rrcConnectionResumeRequest_r13:
        ...
        printf("[eNB] UL CCCH Message Received --- RRCConnectionResumeRequest-NB\n");
    ...
    //FIXME: original resumeid
   
    if(rrcConnectionResumeRequest_NB_IoT->resumeID_r13.buf[0]==0x01&&rrcConnectionResumeRequest_NB_IoT->resumeID_r13.buf[1]==0x01&&rrcConnectionResumeRequest_NB_IoT->resumeID_r13.buf[2]==0x01
      &&rrcConnectionResumeRequest_NB_IoT->resumeID_r13.buf[3]==0x01&&rrcConnectionResumeRequest_NB_IoT->resumeID_r13.buf[4]==0x01)
    {
      rrc_eNB_generate_RRCConnectionResume_NB_IoT(ctxt_pP,
                                 rrc_eNB_get_ue_context_NB_IoT(&eNB_rrc_inst_NB_IoT[ctxt_pP->module_id], ctxt_pP->rnti),
                                 CC_id);
    }else{
       rrc_eNB_generate_RRCConnectionReject_NB_IoT(ctxt_pP,
...
//#if defined(ENABLE_ITTI)...
    ...
    case UL_DCCH_MessageType_NB__c1_PR_rrcConnectionSetupComplete_r13:
        ...
        printf("[eNB]UL DCCH message received --- RRCConnectionSetupComplete-NB)\n");
#ifdef RRC_MSG_PRINT
    ...
    case UL_DCCH_MessageType_NB__c1_PR_securityModeComplete_r13:
    	 ...
    	 */
     printf("[eNB]UL DCCH message received --- SecurityModeComplete-NB)\n");
      ...
      //rrc_eNB_generate_defaultRRCConnectionReconfiguration_NB_IoT(ctxt_pP, ue_context_p);
      //This function needs to coordinate with TASK_PDCP_ENB
      //rrc_eNB_generate_UECapabilityEnquiry_NB_IoT(ctxt_pP, ue_context_p);
      //RRC Connection Release
    //rrc_eNB_generate_RRCConnectionRelease_NB_IoT(ctxt_pP, ue_context_p);
    //RRC Connection Suspend
    rrc_eNB_generate_RRCConnectionSuspend_NB_IoT(ctxt_pP, ue_context_p);
...
//MP: the integrity protection is still not used in OAI --> MAC-I is padded always to 0 so no need to modify it
  ...
  ///no CounterCheckResponse
    ...
    case UL_DCCH_MessageType_NB__c1_PR_rrcConnectionResumeComplete_r13:
      ...
      printf("[eNB] UL DCCH Message Received --- RRCConnectionResumeComplete-NB\n");
