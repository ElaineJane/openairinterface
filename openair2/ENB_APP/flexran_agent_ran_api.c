...
/* TODO should mid_t/module_id_t be harmonized? */
...
uint8_t flexran_get_ue_pmi(mid_t mod_id, mid_t ue_id, uint8_t cc_id)
{
  /* TODO Xenofon to check this -> Robert: is this correct? */
  if (!UE_list[mod_id]) return 0;
...
long flexran_get_tdd_ack_nack_feedback(mid_t mod_id, mid_t ue_id) {
  // TODO: This needs fixing -> Robert: is it right now?
  if (!rrc[mod_id]) return -1;
...
{
  // TODO: need to double check -> Robert: why not give extendedBSR_Sizes_r10?
  if (!rrc[mod_id]) return -1;
  ...
  /* TODO The code previously tested:
   * if (val > 0) {
   *   return 1;
   * }
   */
