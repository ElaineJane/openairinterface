                ...
                if (report_config->ue_report_type[i].ue_report_flags & PROTOCOL__FLEX_UE_STATS_TYPE__FLUST_DL_CQI) {
				    ...
				    csi11->wb_cqi[0] = flexran_get_ue_wcqi (enb_id, i);                                       		    
                                    ...
                                    if (flexran_get_antenna_ports(enb_id, j) == 2 && csi_reports[j]->ri == 1) {
                                        // TODO PMI
                                        csi11->wb_pmi = flexran_get_ue_pmi(enb_id, i, 0);
                                        csi11->has_wb_pmi = 1;
...
int flexran_agent_mac_sf_trigger(mid_t mod_id, const void *params, Protocol__FlexranMessage **msg) {
  ...
  for (i = 0; i < NUMBER_OF_UE_MAX; i++) {
    for (j = 0; j < 8; j++) {
      /* TODO Navid: is this right? 0 is CC, where is HARQ process? */
      if (RC.mac && RC.mac[mod_id] && RC.mac[mod_id]->UE_list.eNB_UE_stats[0][i].harq_pid == 1) {
	available_harq[i] = j;
  ...
  if (sf_trigger_msg->n_dl_info > 0) {
	...
	goto error;
      ...
      /* TODO Navid: is this right? 0 is CC, where is HARQ process? Also
       * contained available_harq[UE_id] */
      if (RC.mac && RC.mac[mod_id])
        RC.mac[mod_id]->UE_list.eNB_UE_stats[0][UE_id].harq_pid = 0;
      ...
      for (j = 0; j < dl_info[i]->n_harq_status; j++) {
        /* TODO Navid: which one is right?
        RC.mac[mod_id]->UE_list.UE_sched_ctrl[i].round[cc_id][j]
        RC.mac[mod_id]->UE_list.eNB_UE_stats[j][i].harq_round */
        dl_info[i]->harq_status[j] = 0;
...
void flexran_agent_init_mac_agent(mid_t mod_id) {
  ...
  for (i = 0; i < NUMBER_OF_UE_MAX; i++) {
    for (j = 0; j < 8; j++) {
      /* TODO Navid: is this right? 0 is CC, where is HARQ process? */
      if (RC.mac && RC.mac[mod_id])
        RC.mac[mod_id]->UE_list.eNB_UE_stats[0][i].harq_pid = 0;
    }
  }
}
